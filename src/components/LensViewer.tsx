...
type Props={datasetId:string;sample:Sample|null;channel:string;defects:Defect[];onChannel:(c:string)=>void;labels:Record<string,string>;hold?:boolean;onHold?:()=>void;selectedDefect?:number;onProbe?:(p:{x:number;y:number;gray:number|null})=>void;processing?:boolean};
export function LensViewer({datasetId,sample,channel,defects,onChannel,labels,hold,onHold,selectedDefect=0,onProbe,processing=false}:Props){
 ...
 const img=sample&&channels.includes(channel)?previewUrl(datasetId,sample.id,channel):'';
 ...
 return <section className={`glassPanel viewerPanel productionPanel premiumViewer ${processing?'processing':''}`}>
   <div className="viewerCompactHeader">
     <div className="viewerTitleInline"><h2>Inspection Viewer</h2><span>{sample?`WT-${String(sample.wt_index).padStart(4,'0')} · P${sample.position}`:'No lens selected'}</span></div>
     ...
     <div className="viewerUtility premiumViewerUtility"><Link href="/storage" title="Image filters"><SlidersHorizontal/><span>Filters</span></Link>{onHold&&<button className={hold?'active':''} onClick={onHold} title={hold?'Release held lens':'Hold current lens'}>{hold?<Lock/>:<Unlock/>}<span>{hold?'Held':'Hold'}</span></button>}</div>
   </div>
