...
export function DatasetLoader({open,onClose,onLoaded}:{open:boolean;onClose:()=>void;onLoaded:(id:string)=>void}){
  ...
  const[name,setName]=useState('Offline OKLIN dataset');
  ...
  return <div className="modalBack">
    <div className="modernModal">
      ...
      <span className="eyebrowText">OFFLINE INPUT</span>
      <h2>Load inspection image folder</h2>
      <p>Use a backend-local path or upload the complete directory tree from your browser. Files with the same base name are grouped into one logical lens.</p>
      ...
      <button className="uploadFolder" onClick={()=>inputRef.current?.click()} disabled={busy}><FolderUp/>Upload folder from browser</button>
